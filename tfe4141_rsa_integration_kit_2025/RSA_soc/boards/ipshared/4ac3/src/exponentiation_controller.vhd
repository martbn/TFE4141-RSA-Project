...
architecture RTL of exponentiation_controller is
    ...
    type state_type is (
        ...
        WINDOW_WAIT_MONT,       -- Wait for Montgomery multiplication completion
        WINDOW_SQUARE_CONTINUE, -- Continue squaring if square_count > 1
        WINDOW_MULT_START,      -- Multiply by precomputed window value
        WINDOW_DONE,            -- Window processing complete, wait for mult
    ...
    signal precomp_base2_written : std_logic := '0';
    signal precomp_base2_written_next : std_logic := '0';
begin
    ...
    if reset_n = '0' then
        ...
        if msb_scan_request = '1' then
            ...
            if key(msb_scan_ptr) = '1' then
                -- Pulse msb_found focr a single clock and stop the scan
                msb_found <= '1';
                msb_scan_active <= '0';
            else
...
begin
    ...
    case state is
        ...
        when WINDOW_WAIT_MONT =>
            -- ===== WINDOW_WAIT_MONT: Wait for Montgomery completion =====
            if montgomery_done = '1' then
                if square_count > 1 then
                    -- More squarings needed
        ...
        when WINDOW_SQUARE_CONTINUE =>
            -- ===== WINDOW_SQUARE_CONTINUE: Continue squaring loop =====
            mont_request <= '1';
            montgomery_A <= P_reg;
            montgomery_B <= P_reg;
            montgomery_N <= modulus;
        ...
        when WINDOW_DONE =>
            -- ===== WINDOW_DONE: Wait for multiplication completion =====
            if montgomery_done = '1' then
                if finalize = '1' then
                    next_state <= FINISHED;
                else
